...
void loop() {
  ...
  if (rVal == rVal2) {
    if (rVal != rasterButtonState) {
      if (rVal == HIGH) {
        Serial.println("Raster button pushed");
        rasterAssayWell(6.0);
      }
      else {
  ...
  if (bVal == bVal2) {
    if (bVal != backwardButtonState) {
      if (bVal == HIGH) {
        Serial.println("Backward button pushed");
...
void solenoidOut() {
    ...
    analogWrite(pinSolenoid, 1023);
}

void solenoidIn() {
    Serial.println("Shifting in...");
    analogWrite(pinSolenoid, 10);
}
...
void rasterAssayWell(double mm) {
  ...
  for (int i = 0; i < steps; i += 1) {
    ...
    if ((i % stepsPerRaster) == 0) {
      solenoidStatus = analogRead(pinSolenoid);
      Serial.println(solenoidStatus);
      if (solenoidStatus > 1020) {
        solenoidOut();
      }
      else {
        solenoidIn();
